/** Optional atomic-append variant; duplicated so the normal renderer keeps its own gzip chunk. */
export const pixelBlockEventComputeShader = /* wgsl */ `
struct Params {
  width: u32,
  height: u32,
  tick: u32,
...
@group(0) @binding(2) var<storage, read_write> eventBuffer: EventBuffer;
...
fn step(@builtin(global_invocation_id) id: vec3<u32>) {
  ...
  if (moved != 0u) {
    ...
    if (slot < params.eventCapacity) {
      eventBuffer.records[slot] = vec4<u32>(1u, topLeft, moved, params.tick);
    } else {
